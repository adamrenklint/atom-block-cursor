...
function updateCursorStyles(config) {
  ...
  for(let key of Object.keys(config)) {
    ...
    updateCursorStyle(config[key]);
  }
}

// update the style for a single cursor style
function updateCursorStyle(cursorStyle) {
  ...
  var blinkOn = assign(getGlobalConfig().blinkOn, cursorStyle.blinkOn);
  var blinkOff = assign(getGlobalConfig().blinkOff, cursorStyle.blinkOff);
  ...
  blinkOn.properties = {
    ...
    transitionDuration: pulseDuration,
    // cursor line fix
    zIndex: cursorLineFix ? 1 : -1
  };
  // blink on border
  ...
  style.innerHTML += createCSSRule(blinkOn) + createCSSRule(blinkOff);
}
...
module.exports = BlockCursor = {
  ...
  activate: function activate() {
    this.disposables = disposables = new CompositeDisposable();
